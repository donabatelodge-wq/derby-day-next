...
//
// If the admin has turned payments off (app_settings.payments_enabled =
// 'false'), this route skips Stripe entirely and applies the same state
// change directly, so testing/demoing the app never requires live Stripe
// keys.
export async function POST(request: NextRequest) {
  try {
    ...
    // Service-role client so the admin-only app_settings write policy can't
    // be bypassed by anything client-side, and so the group state change
    // below (when payments are off) doesn't depend on the caller's own RLS.
    ...
    if (!paymentsEnabled) {
      if (kind === "initial") {
        const { error } = await service
          .from("groups").update({ status: "active" }).eq("id", groupId).eq("status", "pending_payment");
        if (error) {
          return NextResponse.json({ error: `Failed to activate group: ${error.message}` }, { status: 500 });
        }
      } else {
        const { error } = await service
          .from("groups").update({ max_players: targetTier }).eq("id", groupId);
        if (error) {
          return NextResponse.json({ error: `Failed to raise player limit: ${error.message}` }, { status: 500 });
        }
      }
